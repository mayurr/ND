...
function addApplicant() {
	...
	$.ajax({
		...
		data:{givegetId:giveId,comments:comments,action:"AddApplicant"},
		type : 'POST',
...
function showApplicants(id) {
	...
	if(tempArray.length > 1) {
		$("#"+id).addClass("giveRowClickBorder");
		giveId = tempArray[1];
	}
	else {
		$("#giveRow"+id).addClass("giveRowClickBorder");
		giveId = id;
	}

	$.ajax({
		url :"Controller/controller.action.php",
		data:{userId:userId,givegetId:giveId,action:"ShowApplicants"},
		type : 'POST',
		success:function(data){
			dataInfo=eval("("+data+")");
			if(dataInfo.result[0]['empty'] != "empty") {
				...
				for(var i=0;i<dataInfo.result.length;i++){
					...
					if(flag == 1){
						...
						if(dataInfo.result[i]['status'] == "Pending"){
							if(!chosenFlag) {
								row += 		"<td width=100 align=center>"+dataInfo.result[i]['status']+"<br /><br /><button id="+dataInfo.result[i]['applicantId']+" onclick='chooseApplicant(this.id);'>Give</button></td>";
							}
							else {
								row += 		"<td width=100 align=center>"+dataInfo.result[i]['status']+"<br /><br /></td>";
							}
			...
			$("#giveImage").attr('src',"uploads/"+dataInfo.result1[0]['objectImage']);
...
function chooseApplicant(applicantId){
	...
	$.ajax({
		url :"Controller/controller.action.php",
		data:{getterId:sessionUserId,giverId:userId,applicantId:applicantId,givegetId:giveId,type:"give",action:"ChooseUser"},
		type : 'POST',
		success:function(data){
			showApplicants(giveId);
		}
...
function confirmReceipt(applicantId) {
	$.ajax({
		url :"Controller/controller.action.php",
		data:{getterId:sessionUserId,giverId:userId,objectId:objectId,applicantId:applicantId,givegetId:giveId,tableName:"give",action:"ConfirmReceipt"},
		type : 'POST',
		success:function(data){
			$.ajax({
				url :"Controller/controller.move.php",
				data:{getterId:sessionUserId,giverId:userId,action:"Attract"},
				type : 'POST',
				success:function(data){
					$.ajax({
						url :"Controller/controller.history.php",
						data:{getterId:sessionUserId,giverId:userId,givegetId:giveId,objectId:objectId,type:"give",action:"Insert"},
						type : 'POST',
						success:function(data){
							...
							$("#pageContainer").load("page"+tabNumber+".php?flag=1&getterId="+sessionUserId+"&giverId="+userId);
						}
					});
				}
			});
			alert("Congratulations on receiving the Give!");
		}
	});
}
...
function confirmNonReceipt(applicantId) {
	$.ajax({
		url :"Controller/controller.action.php",
		data:{getterId:sessionUserId,giverId:userId,objectId:objectId,applicantId:applicantId,givegetId:giveId,tableName:"give",action:"ConfirmNonReceipt"},
		type : 'POST',
		success:function(data){
			$.ajax({
				url :"Controller/controller.move.php",
				data:{getterId:sessionUserId,giverId:userId,action:"Repel"},
				type : 'POST',
				success:function(data){
					$.ajax({
						url :"Controller/controller.history.php",
...
function withdrawApplication(applicantId) {
	$.ajax({
		...
		type : 'POST',
		success:function(data){
			//alert("You have successfully Withdrawn your Application");
			showApplicants(giveId);
		}
	});
}
...
function addBorder(id) {
	$("#"+id).addClass("giveRowBorder");
}

function removeBorder(id) {
	$("#"+id).removeClass("giveRowBorder");
}
...
function giveData(){
	...
	if(image==""){
		...
		$.ajax({
			url :"Controller/controller.action.php",
			data:{type:dropDown,name:name,description:desc,image:image,startDate:startDate,endDate:endDate,comments:comments,location:location,action:"Give"},
			type : 'POST',
			success:function(data){
				$("#leftContainerContents").hide();
				$("#dropDown").val("Select");
				...
				addNewGiveToList(dataString);
			}
		});
	}		
}

function addNewGiveToList(dataString){
	var tempArray = dataString.split("::");
	var giveId = tempArray[0];
	...
	var row = "";
	row += "<tr>";
	row +=	   "<td class='giveRow' id=giveRow"+giveId+" onmouseover='addBorder(this.id);' onmouseout='removeBorder(this.id);' onclick='showApplicants(this.id);'>";
	row +=	   "<table>";
	row +=	   		"<tr><td><table><tr>";
	row +=	   			"<td width=150 height=100 align=center><img class='giveListImage' width=100 height=100 src=uploads/"+ image +" /></td>";
	row +=	   			"<td width=300 height=100 align=left><p class='giveListDescription'>"+ description +"</p></td>";
	row +=	   		"</tr></table></td></tr>";
	row +=	   		"<tr><td><table><tr>";
	row +=	   			"<td width=150 height=10 align=center><p class='giveListName'>"+ name +"</p></td>";
	row +=	   			"<td width=150 height=10 align=left><p class='giveListStartDate'>Start: "+ startDate +"</p></td>";
	row +=	   			"<td width=150 height=10 align=left><p class='giveListEndDate'>End: "+ endDate +"</p></td>";
	row +=	   		"</tr></table></td></tr>";
	row +=	   "</table>";
	row +=	   "</td>";
	row += "</tr>";
	...
	if($("#giveList").length > 0){
		...
		$("#giveList").prepend(row);
	}
	else{
		$("#loadingImageRight").hide();
		var row = "<table id='giveList'>"+row+"</table>";
