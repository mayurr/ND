...
<?php
	if($imgUploaded){
		$marginTop=0;
		$marginLeft=0;
		$imgSize=getimagesize($upload_image);
		if($imgSize[0]>$imgSize[1]){
			$width=190;
			$height=($width*$imgSize[1])/$imgSize[0];
			$marginTop=(190-$height)/2;
		}
		else{
			$height=190;
			$width=($height*$imgSize[0])/$imgSize[1];
			$marginLeft=(190-$width)/2;
		}
		echo '<a id="example1" href='.$upload_image.' >
		<img src="'.$upload_image.'" border="0" width="'.$width.'" height="'.$height.'" style="margin-top:'.$marginTop.'px; margin-left:'.$marginLeft.'px" /></a>';
	}else{
