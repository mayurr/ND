...
class DatabaseAlt {
   ...
   * @return  None.
   */ 
	public function __construct($host = 'localhost', $database = 'nayadaur', $username = 'root', $password = '', $persistent = FALSE) {
  ...
  public function executeQuery() {
  	// Assignment of set query string to $strQuery
  	$strQuery = $this->query;
  	//echo $strQuery;
      ...
      if($this->querySucceeded()) {
       ...
       // else { // If the query returned a value greater than 0. Return the rows.
          ...
          switch($this->getQueryType()) {
            case 'INSERT':
              return TRUE;//$this->getLastInsertId();
              break;
  ...
  private function runQuery($strSqlStatement = NULL) {
    ...
    if(!is_null($strSqlStatement)) {
      ...
      // Run the query.
      $this->queryResult = mysql_query($strSqlStatement);
